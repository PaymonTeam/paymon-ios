...
BuffersStorage::BuffersStorage(bool threadSafe) {
    ...
    for (uint32_t a = 0; a < 4; a++) {
        freeBuffers8.push_back(new SerializedBuffer((uint32_t) 8));
    }
    for (uint32_t a = 0; a < 5; a++) {
        freeBuffers128.push_back(new SerializedBuffer((uint32_t) 128));
    }
}

SerializedBuffer *BuffersStorage::getFreeBuffer(uint32_t size) {
    uint32_t byteCount = 0;
    std::vector<SerializedBuffer *> *arrayToGetFrom = nullptr;
...
void BuffersStorage::reuseFreeBuffer(SerializedBuffer *buffer) {
    if (buffer == nullptr) {
        return;
    }
    std::vector<SerializedBuffer *> *arrayToReuse = nullptr;
    ...
    if (arrayToReuse != nullptr) {
        ...
        if (arrayToReuse->size() < maxCount) {
            ...
            printf("too more %d buffers", capacity);
            delete buffer;
        }
        if (isThreadSafe) {
            pthread_mutex_unlock(&mutex);
